...
QVariantMap ActivityRunner::ReloadConfiguration()
{
    return {
        {"X-Plasma-Runner-Min-Letter-Count", qMin(m_keyword.count(), m_keywordi18n.count())},
        {"X-Plasma-Runner-Match-Regex", '^' + m_keyword + '|' + m_keywordi18n + " ?"},
    };
}
